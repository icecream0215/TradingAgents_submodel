...
def test_third_party_openai_token_tracking():
    """测试ThirdPartyOpenAI的Token跟踪功能"""
    logger.info("🧪 测试ThirdPartyOpenAI Token跟踪功能...")
    ...
    try:
        ...
        # 生成唯一会话ID
        session_id = f"actual_test_{int(time.time())}"
        ...
        logger.error(f"❌ ThirdPartyOpenAI测试失败: {e}")
        import traceback
...
def main():
    ...
    # 3. 测试ThirdPartyOpenAI Token跟踪
    api_test_result = test_third_party_openai_token_tracking()
    ...
    if api_test_result:
        logger.info("🎉 ThirdPartyOpenAI Token使用量测试成功！")
        logger.info("✅ 系统能够在实际运行时正确获取和记录Token用量数据")
    else:
        logger.error("❌ ThirdPartyOpenAI Token使用量测试失败")
        logger.info("💡 可能的原因:")
