            ...
            if provider.lower() == "openai" and api_key:
                if not self.validate_openai_api_key_format(api_key):
                    ...
                    if hasattr(api_key, 'get_secret_value'):
                        key_display = api_key.get_secret_value()[:10] if api_key.get_secret_value() else 'None'
                    else:
                        key_display = str(api_key)[:10] if api_key else 'None'
                    logger.warning(f"⚠️ OpenAI API密钥格式不正确，将被忽略: {key_display}...")
                    return ""
            return api_key
        return ""
    ...
    def calculate_cost(self, provider: str, model_name: str, input_tokens: int, output_tokens: int) -> float:
        ...
        try:
            ...
            if not config_file.exists():
                logger.warning(f"⚠️ [calculate_cost] Web价格配置文件不存在: {config_file}")
                return 0.0
                
            with open(config_file, 'r', encoding='utf-8') as f:
                web_pricing = json.load(f)
            
            # 优先使用简化的通用配置
            if "input_price" in web_pricing and "output_price" in web_pricing:
                input_price = web_pricing["input_price"]
                output_price = web_pricing["output_price"]
            else:
                # 查找对应的供应商配置（兼容旧格式）
                provider_key = provider.lower()
                if provider_key not in web_pricing:
                    logger.warning(f"⚠️ [calculate_cost] 未找到供应商配置: {provider_key}")
                    logger.debug(f"⚠️ [calculate_cost] 可用的供应商: {list(web_pricing.keys())}")
                    return 0.0
                
                provider_config = web_pricing[provider_key]
                input_price = provider_config.get("input_price", 0.0)
                output_price = provider_config.get("output_price", 0.0)
            
            if input_price == 0.0 and output_price == 0.0:
                logger.warning(f"⚠️ [calculate_cost] 价格为0")
                return 0.0
            ...
            logger.debug(f"💰 [calculate_cost] {provider}: 输入={input_tokens}*{input_price}={input_cost:.6f}, 输出={output_tokens}*{output_price}={output_cost:.6f}, 总计={total_cost:.6f}")
            
            return round(total_cost, 6)
            
        except Exception as e:
            logger.error(f"❌ [calculate_cost] 计算成本失败: {e}")
            return 0.0
