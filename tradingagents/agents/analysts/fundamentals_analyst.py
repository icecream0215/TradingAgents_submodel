...
2. 包含具体的财务数据、估值指标和专业分析
3. 提供明确的投资建议和风险提示
4. 报告长度不少于800字
5. 使用中文撰写

请分析股票{ticker}的基本面情况，包括：
- 公司基本信息分析
- 财务状况评估
- 盈利能力分析
- 估值分析
- 投资建议"""
                    ...
                    analysis_with_data = f"""以下是获取到的基本面数据：
...
{analysis_prompt}"""
                    ...
                    final_result = fresh_llm.invoke([HumanMessage(content=analysis_with_data)])
                    report = final_result.content

                    logger.info(f"📊 [基本面分析师] 生成完整分析报告，长度: {len(report)}")
                    ...
                    # 降级处理：返回工具调用信息
                    report = f"基本面分析师调用了工具但分析生成失败: {tool_calls_info}"
                    return {
                        "messages": [result],
                        "fundamentals_report": report,
                    }
...
- 分析要详细且专业"""
                ...
                try:
                    ...
                    # 直接使用HumanMessage对象而不是字典格式
                    analysis_result = fresh_llm.invoke([HumanMessage(content=analysis_prompt)])
