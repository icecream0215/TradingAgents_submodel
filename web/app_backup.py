            ...
            if latest_id:
                ...
                if (progress_data and
                    ...
                    if formatted_results:
                        ...
                        # 使用格式化结果中的analysis_id，如果没有则使用latest_id
                        analysis_id_from_results = formatted_results.get('analysis_id', latest_id)
                        st.session_state.current_analysis_id = analysis_id_from_results
                        # 检查分析状态
                        ...
                        logger.info(f"📊 [结果恢复] 从分析 {latest_id} 恢复结果，analysis_id: {analysis_id_from_results}，状态: {analysis_status}")
...
def main():
    ...
    # 添加讨论显示切换
    show_discussion = st.sidebar.checkbox("🗣️ 显示模型讨论", value=True, help="显示/隐藏右侧模型讨论实况")
    ...
    # 主内容区域 - 根据是否显示指南和讨论调整布局
    current_analysis_id = st.session_state.get('current_analysis_id')
    should_show_right_panel = show_guide or (show_discussion and current_analysis_id)
    
    if should_show_right_panel:
        col1, col2 = st.columns([2, 1])  # 2:1比例
    else:
        col1 = st.container()
        col2 = None
            ...
            if is_completed and not st.session_state.get('analysis_results') and progress_data:
                if 'raw_results' in progress_data:
                    try:
                        ...
                        if formatted_results:
                            st.session_state.analysis_results = formatted_results
                            # 使用格式化结果中的analysis_id用于讨论追踪
                            analysis_id_from_results = formatted_results.get('analysis_id', current_analysis_id)
                            if analysis_id_from_results != current_analysis_id:
                                st.session_state.current_analysis_id = analysis_id_from_results
                                logger.info(f"📊 [讨论追踪] 更新analysis_id: {current_analysis_id} -> {analysis_id_from_results}")
                            st.session_state.analysis_running = False
    ...
    if col2 is not None:
        with col2:
            ...
            # 如果启用讨论显示且有活跃分析，显示完整的分析实况
            if show_discussion and current_analysis_id:
                # 导入讨论显示组件
                try:
                    from components.discussion_display import display_live_discussion_v2
                    
                    # 使用新版本的统一显示组件（包含进度+讨论+消息流）
                    display_live_discussion_v2(current_analysis_id)
                    
                    # 自动刷新控制
                    auto_refresh = st.checkbox(
                        "🔄 自动刷新", 
                        value=True, 
                        key=f"auto_refresh_{current_analysis_id}",
                        help="自动刷新分析实况内容"
                    )
                    ...
                    st.error(f"分析实况组件加载失败: {e}")
                    
                except Exception as e:
                    st.warning(f"分析实况暂时不可用: {e}")
                    logger.error(f"分析实况显示错误: {e}")
            
            elif show_discussion:
                # 没有活跃分析时显示提示
                st.markdown("### �️ 分析实况")
                st.info("🔄 请先开始分析，以查看实时信息流")
                
            else:
                # 如果关闭了讨论显示，只显示简洁的状态信息
                if current_analysis_id:
                    st.markdown("### � 分析状态")
                    ...
                    # 显示分析ID
                    st.text(f"ID: {current_analysis_id[:8]}...")
                else:
                    st.markdown("### � 分析中心")
                    st.info("💡 开始新的分析以查看实时状态")
                    A: 讨论会在分析开始后逐步显示，请耐心等待或手动刷新。

                    **Q: 模型讨论是实时的吗？**
                    A: 是的，讨论过程会实时更新，您可以看到AI模型的思考过程。
                    """)
